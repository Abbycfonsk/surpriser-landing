...
export function initCreateSurpriseListeners() {
  const cs_skill = document.getElementById("cs_skill");
  const cs_header_image = document.getElementById("cs_header_image");
  const preview_header = document.getElementById("preview_header");
  const cs_header_file = document.getElementById("cs_header_file");

  if (!cs_skill || !cs_header_file) return;

  cs_skill.onchange = function () {
    const imagePath = `https://api.surpriser.app/storage/surprise_headers/category_${this.value}.png`;
